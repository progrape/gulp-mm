...
function _parse(file, content){
    ...
    content = content.replace(reg, function(word, quot, match) {
        ...
        var md5 = _md5(match[0]).substr(0, 6);
        var out = "<%@GetVar(" + md5 + ")%>";
        word = out;
...
function _setPoFile(file, content){
    ...
    for(var poFileName in poFiles){
        ...
        fs.writeFileSync(path.join(path.dirname(path.join(file.cwd, dest, file.relative)).replace(/tmpl$/gi, ''), "lang_" + poFileBase + ".html"), langContent);

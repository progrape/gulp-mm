        ...
        if(regex.test(file.contents.toString())){
            ...
            lodash.forEach(sprites, function (sprite, retina) {
                ...
                lodash(result.items).forEach(function (s) {
                    ...
                    content = content.replace(retina + s.item.abs + '--x--', -s.x);
                    content = content.replace(retina + s.item.abs + '--y--', -s.y);
                });
                ...
                var bsx = retina === '2x' ? ctxWidth[retina] / 2 : ctxWidth[retina];
                var bsy = retina === '2x' ? ctxHeight[retina] / 2 : ctxHeight[retina];

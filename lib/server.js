...
function handler(opts){
    ...
    return function(req, res){
        ...
        if (fs.lstatSync(abs).isDirectory()){
            ...
            var result = fs.readFileSync(__dirname + '/tree.html', 'utf-8');
            var render = '';
            files.forEach(function (file) {
                if(file.charAt(0) === '.'){
                    return;
                }
                var url = (pathname === '/'? '' : pathname) + '/' + file;
                var icon = fs.lstatSync(path.join(abs, file)).isDirectory() ? 'icon_folder' : 'icon_file';
        ...
        // 如果开启了模板编译
        var content = content = fs.readFileSync(abs, 'utf-8');
        if (opts.tmpl && fs.existsSync(opts.tmpl)){
            var context = {};
            try{
                context = JSON.parse(fs.readFileSync(opts.tmpl, 'utf-8'));
            }catch(err){
                throw new Error('parse context file error, please check context file ' + opts.tmpl);
            }
            content = tmpl(content, context, abs);
        }

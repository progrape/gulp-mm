    ...
    return function(req, res){
        ...
        if (opts.live && mine.indexOf('text/html') !== -1){
            var replacement = '<script>{{socketjs}}</script>\n' +
                ...
                '</script>\n' +
                '</body>\n';
            replacement = replacement.replace('{{socketjs}}', socketjs).replace('{{url}}', opts.url);
...
function start(options){
    ...
    opts.url = 'http://127.0.0.1' + (opts.port == 80 ? '' : ':' + opts.port);
    var app = http.createServer(handler(opts));
    var io = socket(app);

    app.listen(opts.port, function(){
        console.log('[' + gutil.colors.green('server') + '] start on %s', opts.url);
    });

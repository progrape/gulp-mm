...
function _funcParse(name, args){
    ...
    if(/if/.test(name)){ // if & else if
        return args.replace(/(.*?)([!=><])(.*)/, function(word, cond, equal, val){ // <%@if($bear$=bear)%> -> <%@if($bear$=='bear')%>
            return name + "(" + _funcArgsParse(cond) + (/[!=]/.test(equal) ? equal + "=" : equal) + _funcArgsParse(val) + ")";
        });
    }
...
function _funcArgsParse(arg){
    ...
    if(argArr){
        ...
        ret.push("'" + arg.replace(/\$(.{0,40})(?:.DATA)?\$/g, "' + $& + '") + "'"); // <%@GetResFullName($images_path$a.jpg)%> -> <%@GetResFullName('' + $images_path$ + 'a.jpg')%>
    }
...
function tmpl(str, data, path) {
    ...
    var fnStr = "var p=[]; with(obj){p.push('" +
        str
            ...
            .replace(/\n/g, "#br#")
            .replace(/<%##.*?##%>/g, "")
            .replace(/<%#include/g, "\finclude")
            .replace(/<%#.*?<%#\/.*?%>/g, "")
            .replace(/<%@/g, "\f")
            .replace(/%>/g, "\r")
            .replace(/'/g, "\\'")

            .replace(/\f(.*?)\((.*?)\)\r/g, function (word, match1, match2) { // <%@StrCmpNoCase(GetDeviceInfo(TYPE),iPad,4)%> -> <%@StrCmpNoCase(GetDeviceInfo('TYPE'),'iPad','4')%>
                return "\f" + _funcParse(match1, match2) + "\r";
            })
            .replace(/(\f.*?)\$(.{0,40})(?:.DATA)?\$(.*?\r)/g, "$1$2$3") // <%@if($xxx$=)%> -> <%@if(xxx=)%>
            .replace(/\$(.{0,40})(?:.DATA)?\$/g, "', $1, '") // <em>$xxx$</em> -> '<em>' + xxx + '</em>'
            .replace(/\f(.*?)\r/g, function(word, match){
